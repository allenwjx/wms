...
/**
 * @author allen
 * @create $ ID: BGAccessDecisionManager, 18/2/24 14:25 allen Exp $
 * @since 1.0.0
 */
public class BGAccessDecisionManager implements AccessDecisionManager {
    ...
    public void decide(Authentication authentication, Object object, Collection<ConfigAttribute> configAttributes) throws AccessDeniedException,
        ...
        if (CollectionUtils.isEmpty(configAttributes)) {
            throw new AccessDeniedException("No rights to access the resource");
        }
        FilterInvocation filterInvocation = FilterInvocation.class.cast(object);

        // 遍历用户的权限，检查是否有可访问该资源的权限
        for (GrantedAuthority authority : authentication.getAuthorities()) {
            RequestMatcher matcher = new AntPathRequestMatcher(authority.getAuthority());
